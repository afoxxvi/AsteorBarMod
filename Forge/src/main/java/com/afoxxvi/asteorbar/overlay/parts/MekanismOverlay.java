...
public class MekanismOverlay extends BaseOverlay {
    private void draw(GuiGraphics guiGraphics, int left, int top, int right, int bottom, double length, boolean flip) {
        ...
        drawEmptyFill(guiGraphics, left + 1, top + 1, right - 1, bottom - 1, emptyColor);
        drawFillFlip(guiGraphics, left + 1, top + 1, right - 1, bottom - 1, energyLength, 0xff3bfb98, flip);
    }
    ...
    public void renderOverlay(RenderGui gui, GuiGraphics guiGraphics, float partialTick, int screenWidth, int screenHeight) {
        ...
        switch (Overlays.style) {
            ...
            case Overlays.STYLE_ABOVE_HOT_BAR_LONG, Overlays.STYLE_ABOVE_HOT_BAR_SHORT -> {
                ...
                gui.leftHeight(6);
                draw(guiGraphics, left, top, left + BOUND_FULL_WIDTH_SHORT, top + 5, length, false);
            }
            case Overlays.STYLE_TOP_LEFT -> {
                int top = Overlays.vertical;
                int left = Overlays.horizontal;
                draw(guiGraphics, left, top, left + Overlays.length, top + 5, length, false);
                Overlays.vertical += 6;
            }
            case Overlays.STYLE_TOP_RIGHT -> {
                int top = Overlays.vertical;
                int left = screenWidth - Overlays.length - Overlays.horizontal;
                draw(guiGraphics, left, top, left + Overlays.length, top + 5, length, true);
                Overlays.vertical += 6;
            }
